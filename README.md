...
На вход в функцию для обучения подаются четыре параметра. Список словарей данных производителя, список словарей дынных диллера для обучения, список словарей данных внешних ключей и название столбца из данных производителя, по которому смотрится описание и ищутся соответствия с данными от диллера, по умолчанию - это столбец **name**, но можно подставить и другой столбец, к примеру и **name_1c**. Функция обучения возвращает два объекта - обученную модель и фрэйм с эмбеддингами данных производителя. Ообученная модель и фрэйм с эмбеддингами сохраняется в самой функции обучения с помощью инструмента для (де)сериализации Pickle.
